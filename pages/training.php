    <div class="row">
        ...
        <div class="col-7 d-flex justify-content-end align-item-end hover-zoom" style="margin-top: 40px;">
            ...
            <div class="h-100 w-100">
                <a href="process/detail.php?id=<?= $post['postID'] ?>&location=training" class="card bg-dark text-white border-0 h-100 mr-4" style="border-radius: 15px; ">
    ...
    <div class="row m-4">
        <div class="col-8">
            <div class="d-flex flex-wrap justify-content-center">
                <?php
                    require_once('database/database.php');
                    $limitPosts = getTraining($_GET['number']);
                    foreach($limitPosts as $post):
                ?>
                
                <div class="card mt-3" style="width: 25.1rem;">
                    <a href="process/detail.php?id=<?= $post['postID'] ?>&location=training" class="text-dark">
                        ...
                        <div class="card-body ">
                            ...
                            <div class="d-flex justify-content-end">
                                <a href="process/edit.php?id=<?= $post['postID'] ?>&location=training" class="text-success mr-1"><i class="fa fa-pencil-square-o"></i></a>
                                <a href="process/delete.php?id=<?= $post['postID'] ?>&location=training" class="text-dark ml-1"><i class="fa fa-trash"></i></a>
                            </div>
                            <?php endif; ?>
                        </div>
                    </a>
            ...
            <div class="">
                <ul class="d-flex justify-content-center list-unstyled" style="font-size: 22px;">
                    <?php 
                        $page = selectAllTraining();
                        $number_of_page = $page;
                        for($page = 1; $page<= $number_of_page; $page++):
                    ?>
                    <li class="m-3"><a href="index.php?page=training&number=<?= $page ?>" class="text-dark"><?= $page ?></a></li>
        ...
        <div class="card col-4 ">
            ...
            <?php
                // require_once('database/database.php');
                $limitPosts = getTrainingPopular();
                // print_r($limitPosts);
                foreach($limitPosts as $post):
            ?>
            <div class="ml-3 mt-4 border-0" style="max-width: 400px;">
                <a href="process/detail.php?id=<?= $post['postID'] ?>&location=training" class="row no-gutters text-dark">
